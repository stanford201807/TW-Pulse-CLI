...
**核心規則**：
1. ✅ **基準價機制** - 追蹤上次成交價
2. ✅ **加碼規則** - 收盤價 ≥ 基準價 × 1.03，買進 1 份
3. ✅ **減碼規則** - 收盤價 ≤ 基準價 × 0.97，賣出 1 份
4. ✅ **資金控管** - 最多持有 10 份
5. ✅ **移動停利** - 從波段最高點回落 20%，全數出場
6. ✅ **防禦機制** - 跌破 MA200 × 0.96，全數清倉
7. ✅ **抄底機制** - RSI <30 後回升，買進第 1 份

**可配置參數**：
- `max_positions`: 最大持倉份數（預設 10）
- `shares_per_position`: 每份股數（預設 1000）
- `add_threshold`: 加碼門檻（預設 1.03）
- `reduce_threshold`: 減碼門檻（預設 0.97）
- `trailing_stop`: 移動停利（預設 0.20）
- `ma200_stop`: MA200 防禦（預設 0.96）
...
【回測參數】
期間：2021-01-25 至 2026-01-25 (1826 天 / 5.0 年)
初始資金：NT$ 1,000,000
...
【交易明細】（顯示前 10 筆，共 47 筆）
2021-01-15 | 買進 1份 @ NT$ 580 | 抄底機制（RSI 回升）
2021-02-03 | 買進 1份 @ NT$ 598 | 加碼（收盤 615 ≥ 基準價 597 × 1.03）
...
```
